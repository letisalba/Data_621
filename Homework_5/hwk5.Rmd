...
Based on the above, the linear model has better model statistics than the poisson and negative binomial models, but the zero-inflated poison is better overall. We'll check the accuracy of each model in a later section, but for now we'll evaluate the modesl for validity in case we want to use them for our predictions.

$~$

While the linear model may have better statistics than the poisson and binomial, we'll need to validate whether the assumptions of the linear model hold:
...
<<<<<<< HEAD
Now we'll see if the Poisson models exhibit any over dispersion, which could lead us to invalidate the models and go with the negative binomial model instead. This can be done by dividing the residual deviance by the degrees of freedom.
=======
...
>>>>>>> 79e01715a5c722a4e1e805553c7709729153c811
...
<<<<<<< HEAD
Since the over dispersion parameter isn't much greater than 1 (general, less than 1.10), this suggests that the poisson model is a good fit, and we can be relatively confident that we don't need to go with the negative binomial.
=======
...
for (i in seq(1, length(tmp_p))) {
    ...
    cleandf1 <- cleandf %>% filter(pred_target == 0)
    cleandf2 <- cleandf %>% filter(is.na(pred_target))
...
cleandf$pred_p <- ilogit(predict(b_mod2, cleandf, interval='prediction'))
cleandf$pred_target <- ifelse(cleandf$pred_p > p_max_acc, NA, 0)
cleandf1 <- cleandf %>% filter(pred_target == 0)
cleandf2 <- cleandf %>% filter(is.na(pred_target))
cleandf2$pred_target <- round(exp(predict(p_mod4, cleandf2, interval ='prediction')), 0)
cleandfnew <- rbind(cleandf1, cleandf2)
...
# Calculate "close" predictions (within one case of the actual number)
cleandfnew$close <- ifelse((cleandfnew$pred_target >= cleandfnew$TARGET - 1) & (cleandfnew$pred_target <= cleandfnew$TARGET + 1), 1, 0)
acc_pct <- round(100 * sum(cleandfnew$close) / nrow(cleandfnew), 1)
print(paste0('Close prediction accuracy (within one case of actual): ', acc_pct, '%'))
...
# Split df into zero cases and non-zero cases
cleandf_eval1 <- cleandf_eval %>% filter(pred_target == 0)
cleandf_eval2 <- cleandf_eval %>% filter(is.na(pred_target))
>>>>>>> 79e01715a5c722a4e1e805553c7709729153c811
