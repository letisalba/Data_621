...
*The `table()` in R can be used to quickly create frequency tables. Below ... *
...
*The R function below will take the data set into a dataframe and returned the accuracy of the predictions. As a result we obtained an accuracy of 80.66%.*
...
*To generate an ROC Curve we need to calculate the True Postive Rate and the False Postive rates across thersholds from 0 to 1.  The True Postive Rate on the Y axis and the False Postive Rate on the X axis for all the threshold values.  We add an abline that represents what the rate would look like if the values where chosen by chance alone.  If a point falls below theh abline it means that your classifer is performing poor than by chance alone.  The AUC is the Area Under the Curve which is metric of classifer performance overall.  A perfect classifer would have an area fo 1 meaning that it a 100% True Positive Rate and a 0% False Postive Rate.  We are using the trapizoid method to estimate the Area Under the Curve*
